...
const scenes: Record<Kind, Scene> = {
  history: {
    nodes: [
      { id: 'a', label: 'A', x: 110, y: 105, message: '创建项目（最早的提交）' },
      { id: 'b', label: 'B', x: 310, y: 105, message: '补充使用说明' },
      { id: 'c', label: 'C', x: 510, y: 105, message: '修正单位标注' },
      { id: 'd', label: 'D', x: 710, y: 105, message: '增加示例（最新的提交）' },
    ],
    edges: [
      { a: 'a', b: 'b' },
      { a: 'b', b: 'c' },
  ...
  branch: {
    ...
    nodes: [
      ...
      { id: 'c', label: 'C', x: 305, y: 205, message: '尝试新界面', tone: 'text-peach', from: 1 },
      { id: 'd', label: 'D', x: 510, y: 205, message: '完善交互', tone: 'text-peach', from: 1 },
      {
        id: 'm',
        label: 'M',
        x: 710,
  ...
  revert: {
    nodes: [
      ...
      { id: 'b', label: 'B', x: 310, y: 105, message: '启用提示音', tone: 'text-red' },
  ...
  'cherry-pick': {
    ...
    nodes: [
      ...
      { id: 'b', label: 'B', x: 305, y: 75, message: '开发新功能', tone: 'text-peach' },
      { id: 'c', label: 'C', x: 510, y: 75, message: '修正拼写', tone: 'text-green' },
  ...
  remote: {
    ...
    nodes: [
      ...
      { id: 'rb', label: 'B', x: 310, y: 75, message: '补充配置', from: 1, enterY: 130 },
      { id: 'rc', label: 'C', x: 510, y: 75, message: '修复错误', from: 1, enterY: 130 },
      { id: 'rd', label: 'D', x: 710, y: 75, message: '同伴的新提交', tone: 'text-peach', from: 2 },
...
const frame = computed(() => Math.max(0, Math.min(props.step, scene.value.captions.length - 1)))
...
<template>
  <figure class="git-history" :data-kind="kind" :data-step="frame">
    <svg :viewBox="`0 0 820 ${tall ? 270 : 225}`" role="img" :aria-label="scene.captions[frame]">
      <g v-for="lane in scene.lanes" :key="lane.label">
        <rect class="lane" x="12" :y="lane.y - 22" width="796" height="122" rx="12" />
      ...
      >
        ...
        <text class="commit-id" :x="node.x" :y="node.y + 6">{{ node.label }}</text>
        <text class="commit-message" :x="node.x" :y="node.y + 43">{{ node.message }}</text>
      </g>
      <g
        v-if="kind === 'history'"
    ...
    <figcaption aria-live="polite">
      <span class="frame-count">{{ frame + 1 }} / {{ scene.captions.length }}</span>
