...
README 给出实验目标，但不提供修复命令。

### Ruff 实验

候选人做一次明显不符合格式规范的修改，运行 Ruff，并比较格式化前后的代码。

### Pyright 实验

候选人给一个声明为整数的变量传入字符串，观察静态类型报告与实际运行结果的区别。

### pytest 实验

候选人修改一个已有函数的返回行为，使测试失败，阅读失败信息并恢复正确行为。

每个实验只引入一个主要变量，避免同时出现格式、类型和行为问题。
