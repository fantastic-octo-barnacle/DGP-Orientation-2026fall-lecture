# 设计文档

## 已确认设计

[`design/`](./design/) 保存当前有效的总体方案：

- [详细设计路线图](./design/roadmap.md)
- [候选人交付仓库需求记录](./design/candidate-delivery.md)
- [候选人交付 v1 验证记录](./design/candidate-delivery-validation.md)
- [共同基础任务](./design/common-foundations.md)
- [培训设计](./design/training.md)
- [Python 起点路线](./design/python-track.md)
- [Rust 起点路线](./design/rust-track.md)
- [最终面试设计](./design/interview.md)
- [评审框架](./design/evaluation-rubric.md)

## 尚未定稿

[`drafts/`](./drafts/) 保存早期项目与面试题草案。它们用于延续讨论，不代表候选人版 spec 或最终面试内容。

Rust 项目配套草案：

- [起始仓库与分层任务](./drafts/projects/rust-starter.md)
- [通信协议与生命周期要求](./drafts/projects/rust-protocol.md)
- [独立参考程序](./drafts/projects/rust-reference-program.md)

## 决策记录

[`adr/`](./adr/) 保存需要长期保持一致的关键决策：

- [以学习潜力为核心选拔目标](./adr/0001-select-for-learning-potential.md)
